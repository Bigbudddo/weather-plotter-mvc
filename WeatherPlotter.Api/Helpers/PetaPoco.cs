...
namespace WeatherPlotter.PetaPoco {
    ...
    [AttributeUsage(AttributeTargets.Property)]
    public class ColumnMapAttribute : ColumnAttribute {

        public string Operation { get; set; }

        public ColumnMapAttribute(string name) : base (name) {
            Operation = "=";
        }

        public ColumnMapAttribute(string name, string operation) : base(name) {
            Operation = operation;
        }
    }

    [AttributeUsage(AttributeTargets.Property)]
    public class ColumnMultiMapAttribute : Attribute {

        public string Operation { get; set; }
        public string[] Names { get; set; }

        public ColumnMultiMapAttribute(string operation, params string[] names) {
            Operation = operation;
            Names = names;
        }

        public ColumnMultiMapAttribute(params string[] names) {
            Names = names;
        }
    }
    ...
    // Specify a table that will GET ALL data (including result columns)
    // useful in WP where we may select ALL data from a view, BUT still need to create/save to a specific table
    [AttributeUsage(AttributeTargets.Class)]
    public class CompleteTableNameAttribute : Attribute {
        public string Value { get; private set; }

        public CompleteTableNameAttribute(string completeTableName) {
            Value = completeTableName;
        }
    }
    ...
    public class TableInfo {
        ...
        public string CompleteTableName { get; set; }
    ...
    public class Database : IDisposable, IDatabase {
        ...
        public static string ProcessWpDatabaseTables(string _sql) {
            //try {
            //    string dbName = string.Empty;

            //    // check for dev wealthpro database
            //    dbName = GetWpDatabaseTableName(ConfigurationManager.ConnectionStrings["wealthpro"].ConnectionString);
            //    _sql = _sql.Replace("wealthpro_live.", string.Format("{0}.", dbName));

            //    // check for attachments
            //    dbName = GetWpDatabaseTableName(ConfigurationManager.ConnectionStrings["attachments"].ConnectionString);
            //    _sql = _sql.Replace("wealthpro_attachments.", string.Format("{0}.", dbName));

            //    // check for dev reports database
            //    dbName = GetWpDatabaseTableName(ConfigurationManager.ConnectionStrings["reports"].ConnectionString);
            //    _sql = _sql.Replace("wealthpro_reports.", string.Format("{0}.", dbName));
            //}
            //catch { }
            return _sql;
        }

        private static string GetWpDatabaseTableName(string connectionString) {
            string connStringSub = connectionString.Substring(connectionString.IndexOf("database="), connectionString.Length - connectionString.IndexOf("database=") - 1);
            string[] aConnStringSub = connStringSub.Split(';');

            string dbName = aConnStringSub.First();
        ...
        public List<T> Fetch<T>() {
            var pd = PocoData.ForType(typeof(T));
            var sql = "SELECT * FROM " + pd.TableInfo.CompleteTableName;
        ...
        private IEnumerable<T> Query<T>(T instance, Sql s) {
            var sql = s.SQL;
        ...
        public IEnumerable<T> GetByIds<T>(params object[] primaryKeys) {
            return Query<T>(string.Format("WHERE {0} IN (@0)",
                EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.PrimaryKey)), primaryKeys);
        }
        ...
        public List<object> InsertAll<T>(List<T> pocos) {
            List<object> inserted = new List<object>();

            for (int i = 0; i < pocos.Count(); i++) {
                var pd = PocoData.ForType(pocos[i].GetType());
                inserted.Add(Insert(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, pd.TableInfo.AutoIncrement, pocos[i]));
            }
        ...
        public class PocoData {
            ...
            public PocoData(Type t) {
                ...
                // Get table ALL name
                var b = t.GetCustomAttributes(typeof(CompleteTableNameAttribute), true);
                if (b.Length == 0 && a.Length > 0) {
                    TableInfo.CompleteTableName = (a[0] as TableNameAttribute).Value;
                }
                else if (b.Length == 0 && a.Length == 0) {
                    TableInfo.CompleteTableName = t.Name;
                }
                else {
                    TableInfo.CompleteTableName = (b[0] as CompleteTableNameAttribute).Value;
                }
    ...
    public class Sql {
        ...
        private void Build(StringBuilder sb, List<object> args, Sql lhs) {
            if (!String.IsNullOrEmpty(_sql)) {
                ...
                sql = Database.ProcessWpDatabaseTables(sql);

                if (Is(lhs, "WHERE ") && Is(this, "WHERE "))
                    sql = "AND " + sql.Substring(6);

                //sql = (Is(this, "WHEREOR ")) ? "OR " + sql.Substring(8) : "WHERE " + sql.Substring(8);
                // todo; this might need cleaning!
                // note; added by S-Harrison to make query builder a bit more flexible
                if (Is(lhs, "WHEREOR ") && Is(this, "WHEREOR ")) {
                    sql = "OR " + sql.Substring(8);
                }
                else if (Is(lhs, "WHEREOR ") && Is(this, "WHERE ")) {
                    sql = "OR " + sql.Substring(8);
                }
                else if (Is(this, "WHEREOR ")) {
                    sql = "WHERE " + sql.Substring(8);
                }
                // end
        ...
        public Sql OR(string sql, params object[] args) {
